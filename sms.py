...
def update_student():
    ...
    idEntry.grid(row=0, column=1, pady=15, padx=20)
    ...
    nameEntry.grid(row=1, column=1, pady=15, padx=20)
        ...
        if idEntry.get()=='' or nameEntry.get()=='' or phoneEntry.get()=='' or emailEntry.get()=='' or addressEntry.get()=='' or genderEntry.get()=='' or dobEntry.get()=='':
            ...
            try:
                ...
                mycursor.execute(query,(idEntry.get(),nameEntry.get(),phoneEntry.get(),emailEntry.get(),
                                        addressEntry.get(), genderEntry.get(), dobEntry.get(),date,current_time))
                con.commit()
...
def connect_database():
    def connect():
        ...
        try:
            query='create database managementsystem'
            mycursor.execute(query)
            query='use managementsystem'
            mycursor.execute(query)
...
s ='Student Management System' # s[count]= S when count value is 1
...
style=ttk.Style()
style.configure('Treeview', rowheight=30, font=('arial', 13, 'bold'), background= 'white')
